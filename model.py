...
class EE_GAT_ours(nn.Module):
    def __init__(self, args, dep_tag_num, pos_tag_num):
        super(EE_GAT_ours, self).__init__()
        self.args = args
    ...
    def forward(self, tokens_ids, pos_class, dep_ids,text_len, level,adj,adj_node_type):
        ...
        pos_feature = self.dep_embed(pos_class) # (N, L, D)
...
class EE_GAT_only(nn.Module):
    def __init__(self, args, dep_tag_num, pos_tag_num):
        super(EE_GAT_only, self).__init__()
        self.args = args
        ...
        # if args.gat:
        self.gat_attr = [AttrAttentionLayer(2*args.hidden_size,2*args.hidden_size,args,concat=True) for i in range(args.num_heads)]
        self.out_att = AttrAttentionLayer(2*args.hidden_size, 2*args.hidden_size, args, concat=False)
    ...
    def forward(self, ids, tokens_ids, pos_class, dep_ids,text_len, level, adj):

        attr_feature = self.embed(tokens_ids)  # (N, L, D)
        attr_feature = self.dropout(attr_feature)

        attr_feature, _ = self.bilstm(attr_feature) # (N, L, 2*M) M=args.hidden_size
        
        attr_out = [g(attr_feature,adj,level).unsqueeze(0) for g in self.gat_attr]  # (1,N,L,2*M) * num_heads
        attr_out = torch.cat(attr_out, dim=0)  # (num_heads,N,L,2*M)
        attr_out = attr_out.mean(dim=0)  # (N,L,2*M)

       
        x = self.dropout(attr_out)
        x = F.relu(self.out_att(x, adj,level))  # (N,L,2*M)
        x = self.fcs(x) # (N,L,M)
        logit = self.fc_final(x)  #x (N,L,num_class)
        logit = logit.view(logit.shape[0]*logit.shape[1],logit.shape[2])
        return logit
