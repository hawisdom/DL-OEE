...
def load_datasets_and_vocabs(args):
    ...
    if os.path.exists(train_data_file) and os.path.exists(test_data_file):
        ...
        # remove_stop_word_nodes(train_trees)
        # remove_stop_word_nodes(test_trees)
        ...
        del train,test,train_trees,test_trees
        gc.collect()
    # write_unroll_datas(train_all_unrolled, args,'train')
    # write_unroll_datas(test_all_unrolled, args,'test')
...
def write_unroll_datas(datas,args,type):
    file_path = os.path.join(args.dataset_path,args.dataset_name+'_unroll_'+type+'.txt')
    with open(file_path,'w',encoding='utf-8-sig') as f:
        for data in datas:
            for d in data:
                line = str(d.identifier) + '\t' + d.tag + '\t' + d.data.pos + '\t' + str(d.data.parent) + '\t' + d.data.dep + '\t'
                if d.data.isPred == 'Y':
                    line = line + d.data.isPred + '\t'
                else:
                    line = line + '_' + '\t'
                if d.data.A0:
                    line = line + 'A0' + '\n'
